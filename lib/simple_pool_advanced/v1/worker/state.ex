#-------------------------------------------------------------------------------
# Author: Keith Brings
# Copyright (C) 2018 Noizu Labs, Inc. All rights reserved.
#-------------------------------------------------------------------------------

defmodule Noizu.SimplePoolAdvanced.Worker.State do
alias Noizu.SimplePoolAdvanced.Worker.State
  @type t :: %State{
    initialized: boolean,
    migrating: boolean,
    worker_ref: tuple,
    inner_state: any,
    last_activity: any,
    extended: any
  }

  defstruct [
    initialized: false,
    migrating: false,
    worker_ref: nil,
    inner_state: nil,
    last_activity: nil,
    extended: %{}
  ]

  defimpl Inspect, for: Noizu.SimplePoolAdvanced.Worker.State do
    import Inspect.Algebra
    def inspect(entity, opts) do
      heading = "#Worker.State(#{inspect entity.worker_ref})"
      {separator, end_separator} = if opts.pretty, do: {"\n   ", "\n"}, else: {" ", " "}
      inner = cond do
        opts.limit == :infinity ->
          concat(["<#{separator}", to_doc(Map.from_struct(entity), opts), "#{end_separator}>"])
        true -> "<>"
      end
      concat [heading, inner]
    end # end inspect/2
  end # end defimpl

end
