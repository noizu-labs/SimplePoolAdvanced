...
defmodule Noizu.SimplePoolAdvanced.V3.MessageProcessingBehaviour do
  require Logger
  ...
  defmodule Default do
    ...
    #----------------------------------------------
    def __delegate_call_handler(m, _envelope = {:spawn, call = {_msg_type, _call, context}}, from, %Noizu.SimplePoolAdvanced.Worker.State{initialized: :delayed_init} = state) do
      m.__delegate_call_handler({:passive, call}, from, m.delayed_init(state, context))
    end
    def __delegate_call_handler(m, _envelope = {:spawn, call = {_, _call, context}}, from, %Noizu.SimplePoolAdvanced.Worker.State{initialized: false} = state) do
      case m.pool_worker_state_entity().load(state.worker_ref, context, %{}) do
        nil -> {:reply, :error, state}
        inner_state -> m.__delegate_call_handler(m, {:passive, call}, from, %Noizu.SimplePoolAdvanced.Worker.State{state| initialized: true, inner_state: inner_state})
      end
    end
    ...
    def __delegate_call_handler(m, call, from, state = %{inner_state: %{__struct__: inner_module}}) do
      if m.meta()[:inactivity_check] do
        ...
        case inner_module.__call_handler(call, from, state.inner_state) do
          {:reply, response, inner_state} -> {:reply, response, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state, last_activity: l}}
          {:reply, response, inner_state, hibernate} -> {:reply, response, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state, last_activity: l}, hibernate}
          {:stop, reason, inner_state} -> {:stop, reason, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state, last_activity: l}}
          {:stop, reason, response, inner_state} -> {:stop, reason, response, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state, last_activity: l}}
          {:noreply, inner_state} -> {:noreply, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state, last_activity: l}}
          {:noreply, inner_state, hibernate} -> {:noreply, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state, last_activity: l}, hibernate}
        end
      else
        case inner_module.__call_handler(call, from, state.inner_state) do
          {:reply, response, inner_state} -> {:reply, response, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state}}
          {:reply, response, inner_state, hibernate} -> {:reply, response, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state}, hibernate}
          {:stop, reason, inner_state} -> {:stop, reason, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state}}
          {:stop, reason, response, inner_state} -> {:stop, reason, response, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state}}
          {:noreply, inner_state} -> {:noreply, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state}}
          {:noreply, inner_state, hibernate} -> {:noreply, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state}, hibernate}
        end
      end
    end
    ...
    #----------------------------------------------
    def __delegate_cast_handler(m, _envelope = {:spawn, call = {_msg_type, _call, context}}, %Noizu.SimplePoolAdvanced.Worker.State{initialized: :delayed_init} = state) do
      m.__delegate_cast_handler({:passive, call}, m.delayed_init(state, context))
    end
    def __delegate_cast_handler(m, _envelope = {:spawn, call = {_, _call, context}}, %Noizu.SimplePoolAdvanced.Worker.State{initialized: false} = state) do
      case m.pool_worker_state_entity().load(state.worker_ref, context, %{}) do
        nil -> {:reply, :error, state}
        inner_state -> m.__delegate_cast_handler(m, {:passive, call}, %Noizu.SimplePoolAdvanced.Worker.State{state| initialized: true, inner_state: inner_state})
      end
    end
    def __delegate_cast_handler(m, call, state = %{inner_state: %{__struct__: inner_module}}) do
      if m.meta()[:inactivity_check] do
        ...
        case inner_module.__cast_handler(call, state.inner_state) do
          {:reply, _response, inner_state} -> {:noreply, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state, last_activity: l}}
          {:reply, _response, inner_state, hibernate} -> {:noreply, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state, last_activity: l}, hibernate}
          {:stop, reason, inner_state} -> {:stop, reason, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state, last_activity: l}}
          {:stop, reason, _response, inner_state} -> {:stop, reason, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state, last_activity: l}}
          {:noreply, inner_state} -> {:noreply, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state, last_activity: l}}
          {:noreply, inner_state, hibernate} -> {:noreply, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state, last_activity: l}, hibernate}
        end
      else
        case inner_module.__cast_handler(call, state.inner_state) do
          {:reply, _response, inner_state} -> {:noreply, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state}}
          {:reply, _response, inner_state, hibernate} -> {:noreply, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state}, hibernate}
          {:stop, reason, inner_state} -> {:stop, reason, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state}}
          {:stop, reason, _response, inner_state} -> {:stop, reason, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state}}
          {:noreply, inner_state} -> {:noreply, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state}}
          {:noreply, inner_state, hibernate} -> {:noreply, %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state}, hibernate}
        end
      end
    ...
    #----------------------------------------------
    def __delegate_info_handler(m, _envelope = {:spawn, call = {_msg_type, _call, context}}, %Noizu.SimplePoolAdvanced.Worker.State{initialized: :delayed_init} = state) do
      m.__delegate_info_handler({:passive, call}, m.delayed_init(state, context))
    end
    def __delegate_info_handler(m, _envelope = {:spawn, call = {_, _call, context}}, %Noizu.SimplePoolAdvanced.Worker.State{initialized: false} = state) do
      case m.pool_worker_state_entity().load(state.worker_ref, context, %{}) do
        nil -> {:reply, :error, state}
        inner_state -> m.__delegate_info_handler(m, {:passive, call}, %Noizu.SimplePoolAdvanced.Worker.State{state| initialized: true, inner_state: inner_state})
      end
    end
  ...
  defmodule DefaultProvider do
    defmacro __using__(_options) do
      quote do
        require Logger
        @module __MODULE__
        @behaviour Noizu.SimplePoolAdvanced.V3.MessageProcessingBehaviour
        alias Noizu.SimplePoolAdvanced.V3.MessageProcessingBehaviour.Default
        #===============================================================================================================
        # Call routing
        #===============================================================================================================
