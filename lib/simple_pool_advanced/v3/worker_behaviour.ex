...
defmodule Noizu.SimplePoolAdvanced.V3.WorkerBehaviour do
  @moduledoc """
  ...
  defmodule Default do
    ...
    def init(pool_worker, {:migrate, ref, initial_state, context}) do
      ...
      {:ok, %Noizu.SimplePoolAdvanced.Worker.State{extended: %{set_node_task: r || task}, initialized: :delayed_init, worker_ref: ref, inner_state: {:transfer, initial_state}}}
    end
    ...
    def init(pool_worker, {ref, context}) do
      ...
      # TODO V2 version needed.
      {:ok, %Noizu.SimplePoolAdvanced.Worker.State{extended: %{set_node_task:  r || task}, initialized: :delayed_init, worker_ref: ref, inner_state: :start}}
    end
    ...
    def delayed_init(pool_worker, state, context) do
        ...
        # @TODO - investigate strategies for avoiding keeping full state in child def. Aka put into state that accepts a transfer/reads a transfer form a table, etc.
                 {:transfer, {:state, initial_state, :time, time}} ->
                   ...
                   if time < cut_off do
                     ...
                     %Noizu.SimplePoolAdvanced.Worker.State{initialized: initialized, worker_ref: ref, inner_state: inner_state}
                   else
                     if (mod.verbose()) do
                     ...
                     %Noizu.SimplePoolAdvanced.Worker.State{initial_state| initialized: initialized, worker_ref: ref, inner_state: inner_state}
                   end
      ...
      if inactivity_check do
        ustate = %Noizu.SimplePoolAdvanced.Worker.State{ustate| last_activity: :os.system_time(:seconds)}
  ...
  defmacro __using__(options) do
    implementation = Keyword.get(options || [], :implementation, Noizu.SimplePoolAdvanced.V3.WorkerBehaviour.Default)
    ...
    quote do
      ...
      require Logger
      #@behaviour Noizu.SimplePoolAdvanced.WorkerBehaviour
      ...
      @file unquote(__ENV__.file) <> ":#{unquote(__ENV__.line)}" <> "(via #{__ENV__.file}:#{__ENV__.line})"
      use Noizu.SimplePoolAdvanced.V3.SettingsBehaviour.Inherited, unquote([option_settings: option_settings])
      ...
      @file unquote(__ENV__.file) <> ":#{unquote(__ENV__.line)}" <> "(via #{__ENV__.file}:#{__ENV__.line})"
      def init(arg) do
        Noizu.SimplePoolAdvanced.V3.WorkerBehaviour.Default.init(__MODULE__, arg)
      end

      @file unquote(__ENV__.file) <> ":#{unquote(__ENV__.line)}" <> "(via #{__ENV__.file}:#{__ENV__.line})"
      def delayed_init(state, context) do
        Noizu.SimplePoolAdvanced.V3.WorkerBehaviour.Default.delayed_init(__MODULE__, state, context)
      end
      #-------------------------------------------------------------------------
      ...
      def schedule_migrate_shutdown(context, state) do
        Noizu.SimplePoolAdvanced.V3.WorkerBehaviour.Default.schedule_migrate_shutdown(@migrate_shutdown_interval_ms, context, state)
      end

      @file unquote(__ENV__.file) <> ":#{unquote(__ENV__.line)}" <> "(via #{__ENV__.file}:#{__ENV__.line})"
      def clear_migrate_shutdown(state) do
        Noizu.SimplePoolAdvanced.V3.WorkerBehaviour.Default.clear_migrate_shutdown(state)
      end
      ...
      def schedule_inactivity_check(context, state) do
        Noizu.SimplePoolAdvanced.V3.WorkerBehaviour.Default.schedule_inactivity_check(@check_interval_ms, context, state)
      end

      @file unquote(__ENV__.file) <> ":#{unquote(__ENV__.line)}" <> "(via #{__ENV__.file}:#{__ENV__.line})"
      def clear_inactivity_check(state) do
        Noizu.SimplePoolAdvanced.V3.WorkerBehaviour.Default.clear_inactivity_check(state)
      end
      ...
      def load!(state, _args, _from, context, opts \\ nil) do
        ...
        state = %Noizu.SimplePoolAdvanced.Worker.State{state| inner_state: inner_state, initialized: inner_state && true || false}
      ...
      def call_router_catchall(envelope, from, state) do
        Noizu.SimplePoolAdvanced.V3.MessageProcessingBehaviour.Default.__delegate_call_handler(__MODULE__, envelope, from, state)
      end
      def cast_router_catchall(envelope, state) do
        Noizu.SimplePoolAdvanced.V3.MessageProcessingBehaviour.Default.__delegate_cast_handler(__MODULE__, envelope, state)
      end
      def info_router_catchall(envelope, state) do
        Noizu.SimplePoolAdvanced.V3.MessageProcessingBehaviour.Default.__delegate_info_handler(__MODULE__, envelope, state)
      end
