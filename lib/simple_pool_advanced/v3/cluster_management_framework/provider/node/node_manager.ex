...
#-------------------------------------------------------------------------------
defmodule Noizu.SimplePoolAdvanced.V3.ClusterManagementFramework.Cluster.NodeManager do

  use Noizu.SimplePoolAdvanced.V3.StandAloneServiceBehaviour,
      default_modules: [:pool_supervisor, :monitor],
      worker_state_entity: Noizu.SimplePoolAdvanced.V3.ClusterManagementFramework.Cluster.NodeManager.WorkerEntity,
      verbose: false
  ...
  def health_report(node, context, _options \\ %{}) do
    c = Noizu.SimplePoolAdvanced.V3.ClusterManagement.Cluster.Node.State.Repo.get!(node, context) # c = Noizu.SimplePoolAdvanced.V3.ClusterManagement.Cluster.Node.State.Repo.cached(node, context)
    c && c.health_report
  end
  ...
  #--------------------------
  defdelegate service_definition(service, context, options \\ %{}), to: Noizu.SimplePoolAdvanced.V3.ClusterManagementFramework.ClusterManager

  #--------------------------
  # service_status_details/3
  #--------------------------
  defdelegate service_status_details(service, context, options \\ %{}), to: Noizu.SimplePoolAdvanced.V3.ClusterManagementFramework.ClusterManager

  #--------------------------
  # block_for_service_state/5
  #--------------------------
  defdelegate block_for_service_state(service, desired_state, context, options, timeout \\ :infinity), to: Noizu.SimplePoolAdvanced.V3.ClusterManagementFramework.ClusterManager

  #--------------------------
  # block_for_service_status/5
  #--------------------------
  defdelegate block_for_service_status(service, desired_status, context, options, timeout \\ :infinity), to: Noizu.SimplePoolAdvanced.V3.ClusterManagementFramework.ClusterManager
  ...
  defmodule Server do
    @vsn 1.0
    alias Noizu.SimplePoolAdvanced.V3.Server.State
    ...
    def initial_state({node_name, configuration}, context) do
      ...
      node_state = case Noizu.SimplePoolAdvanced.V3.ClusterManagement.Cluster.Node.State.Entity.entity!(node_name) do
        v =  % Noizu.SimplePoolAdvanced.V3.ClusterManagement.Cluster.Node.State.Entity{} ->
          v
          |> Noizu.SimplePoolAdvanced.V3.ClusterManagement.Cluster.Node.State.Entity.reset(context, configuration)
          |> Noizu.SimplePoolAdvanced.V3.ClusterManagement.Cluster.Node.State.Repo.update!(context)
        _ -> Noizu.SimplePoolAdvanced.V3.ClusterManagement.Cluster.Node.State.Entity.ref(node_name)
      end
    ...
    def call__bring_node_online(state, _instructions, context, options \\ %{}) do
      ...
      service_definitions = Noizu.SimplePoolAdvanced.V3.ClusterManagementFramework.ClusterManager.service_definitions(context) || %{}
          ...
          case pool_supervisor().add_child_supervisor(v.service.pool_supervisor(), v.launch_parameters, context) do
            {:ok, pid} ->
              cond do
                ...
                true ->
                  put_in(acc, [v.service], Noizu.SimplePoolAdvanced.V3.ClusterManagement.Cluster.Service.Instance.Status.new(v.service, state.entity.identifier, pid, nil))
                  |> put_in([v.service, Access.key(:status)], :initializing)
                  |> put_in([v.service, Access.key(:state)], :initializing)
                  |> put_in([v.service, Access.key(:pending_state)], :online)
              end
            e ->
              ...
              cond do
                acc[v.service] ->
                  ...
                  |> put_in([v.service, Access.key(:pending_state_changed_on)], DateTime.utc_now())
                true ->
                  put_in(acc, [v.service], Noizu.SimplePoolAdvanced.V3.ClusterManagement.Cluster.Service.Instance.Status.new(v.service, state.entity.identifier, nil, e))
                  |> put_in([v.service, Access.key(:status)], :error)
                  |> put_in([v.service, Access.key(:state)], :initializing)
                  |> put_in([v.service, Access.key(:pending_state)], :online)
              end
      ...
      Noizu.SimplePoolAdvanced.V3.ClusterManagement.Cluster.Node.State.Repo.update!(state.entity, Noizu.ElixirCore.CallingContext.system(context))
