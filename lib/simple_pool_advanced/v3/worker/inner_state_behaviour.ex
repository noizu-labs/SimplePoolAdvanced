...
defmodule Noizu.SimplePoolAdvanced.V3.InnerStateBehaviour do
  @moduledoc """
  ...
  @callback terminate_hook(reason :: any,  Noizu.SimplePoolAdvanced.Worker.State.t) :: {:ok, Noizu.SimplePoolAdvanced.Worker.State.t}
  @callback shutdown(Noizu.SimplePoolAdvanced.Worker.State.t, context :: any, options :: any, from :: any) :: {:ok | :wait, Noizu.SimplePoolAdvanced.Worker.State.t}
  ...
  defmacro __using__(options) do
    ...
    quote do
      ...
      @behaviour Noizu.SimplePoolAdvanced.V3.InnerStateBehaviour
      ...
      #-------------------------------------------------------------------------------
      def reload!(%Noizu.SimplePoolAdvanced.Worker.State{} = state, context, options) do
        case load(state.worker_ref, context, options) do
          nil -> {:reply, :error, state}
          inner_state ->
            {:reply, :ok, %Noizu.SimplePoolAdvanced.Worker.State{state| initialized: true, inner_state: inner_state, last_activity: :os.system_time(:seconds)}}
        end
      end
      ...
      #
      #----------------------------------
      def shutdown(%Noizu.SimplePoolAdvanced.Worker.State{} = state, _context \\ nil, options \\ nil, _from \\ nil), do: {:ok, state}

      #---------------------------------
      #
      #---------------------------------
      def migrate_shutdown(%Noizu.SimplePoolAdvanced.Worker.State{} = state, _context \\ nil), do: {:ok, state}

      #---------------------------------
      #
      #---------------------------------
      def on_migrate(_rebase, %Noizu.SimplePoolAdvanced.Worker.State{} = state, _context \\ nil, _options \\ nil), do: {:ok, state}
