...
defmodule Noizu.SimplePoolAdvanced.V3.PoolBehaviour do
  ...
  defmacro __using__(options) do
    implementation = Keyword.get(options || [], :implementation, Noizu.SimplePoolAdvanced.V3.PoolBehaviour.Default)
    ...
    quote do
      ...
      @behaviour Noizu.SimplePoolAdvanced.V3.PoolBehaviour

      @implementation unquote(implementation)
      @module __MODULE__
      @max_supervisors unquote(max_supervisors)

      use Noizu.SimplePoolAdvanced.V3.SettingsBehaviour.Base, unquote([option_settings: option_settings])
      ...
      if (unquote(default_modules.worker)) do
        defmodule Worker do
          use Noizu.SimplePoolAdvanced.V3.WorkerBehaviour, unquote(options.worker_options)
        end
      end

      if (unquote(default_modules.server)) do
        defmodule Server do
          use Noizu.SimplePoolAdvanced.V3.ServerBehaviour, unquote(options.server_options)
      ...
      if (unquote(default_modules.worker_supervisor)) do
        defmodule WorkerSupervisor do
          use Noizu.SimplePoolAdvanced.V3.WorkerSupervisorBehaviour, unquote(options.worker_supervisor_options)
        end
      end

      if (unquote(default_modules.pool_supervisor)) do
        defmodule PoolSupervisor do
          use Noizu.SimplePoolAdvanced.V3.PoolSupervisorBehaviour, unquote(options.pool_supervisor_options)
        end
      end

      if (unquote(default_modules.monitor)) do
        defmodule Monitor do
          use Noizu.SimplePoolAdvanced.V3.MonitorBehaviour, unquote(options.monitor_options)
        end
      end

      if (unquote(default_modules.record_keeper)) do
        defmodule RecordKeeper do
          use Noizu.SimplePoolAdvanced.V3.RecordKeeperBehaviour, unquote(options.record_keeper_options)
        end
      end
