...
defmodule Noizu.SimplePool.V3.SettingsBehaviour do
  @moduledoc """
    The Noizu.SimplePool.V3.Behaviour provides the entry point for Worker Pools.
    The developer will define a pool such as ChatRoomPool that uses the Noizu.SimplePool.V3.Behaviour Implementation
  ...
  defmodule Base do
    defmacro __using__(opts) do
      ...
      quote do
        @behaviour Noizu.SimplePool.V3.SettingsBehaviour
        @module __MODULE__
        @module_str "#{@module}"
        ...
        @pool_dispatch_table Noizu.SimplePool.V3.SettingsBehaviour.Default.expand_table(@pool, unquote(options.dispatch_table), DispatchTable)

        @options unquote(Macro.escape(options))
        @option_settings unquote(Macro.escape(option_settings))

        @pool_worker_state_entity Noizu.SimplePool.V3.SettingsBehaviour.Default.pool_worker_state_entity(@pool, unquote(pool_worker_state_entity))

        @short_name Module.split(__MODULE__) |> Enum.slice(-1..-1) |> Module.concat()


        defdelegate wait_for_condition(condition, timeout \\ :infinity), to: Noizu.SimplePool.V3.SettingsBehaviour.Default

        def short_name(), do: @short_name

        def profile_start(%{meta: _} = state, profile \\ :default) do
          Noizu.SimplePool.V3.SettingsBehaviour.Default.profile_start(state, profile)
        end

        def profile_end(%{meta: _} = state, profile \\ :default, opts \\ %{info: 100, warn: 300, error: 700, log: true}) do
          Noizu.SimplePool.V3.SettingsBehaviour.Default.profile_end(state, profile, short_name(), opts)
        end
        ...
        defdelegate banner(header, msg), to: Noizu.SimplePool.V3.SettingsBehaviour.Default
        ...
        """
        def meta(), do: Noizu.SimplePool.V3.SettingsBehaviour.Default.meta(@module)

        @doc """
        Append new entries to meta data (internally a map merge is performed).
        """
        def meta(update), do: Noizu.SimplePool.V3.SettingsBehaviour.Default.meta(@module, update)

        @doc """
        Initial Meta Information for Module.
        """
        def meta_init(), do: Noizu.SimplePool.V3.SettingsBehaviour.Default.meta_init(@module, %{stand_alone: @stand_alone})
